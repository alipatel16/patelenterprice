...
// ── Initialise both apps (safe for hot-reload) ────────────────────────────────
const electronicsApp = getApps().find((a) => a.name === "electronics")
  ?? initializeApp(electronicsConfig, "electronics");

const furnitureApp = getApps().find((a) => a.name === "furniture")
  ?? initializeApp(furnitureConfig, "furniture");

// ── Per-category getters ──────────────────────────────────────────────────────
export const getFirebaseApp   = (cat) => cat === "furniture" ? furnitureApp   : electronicsApp;
export const getDbForCategory = (cat) => getFirestore(getFirebaseApp(cat));
export const getAuthForCategory = (cat) => getAuth(getFirebaseApp(cat));

// ── Convenience defaults (electronics) ───────────────────────────────────────
